...
![](https://upload-images.jianshu.io/upload_images/15777037-8930b366c60b8a7a.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
虽然Python的对象引用不涉及类型，变量对于对象来说只是特定命名空间中的名称；类型信息是存储在对象自身中的。但是，Python被认为是强类型语言，如下：
![](https://upload-images.jianshu.io/upload_images/15777037-b39759041a8f7836.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
上图的代码在Java语言中`5`会被隐式转换为字符串，输出`55`，而Python中则会报错，这意味着Python中所有的对象都拥有一个指定的类型（或类）,同时，有一些隐式的转换只在某些特定、明显的情况下发生：
![](https://upload-images.jianshu.io/upload_images/15777037-be1d4aea8ee6bbfd.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

### Python中如何检查一个对象是否是特定类型
利用`isinstance`函数来检查:
![](https://upload-images.jianshu.io/upload_images/15777037-4cf1c437918bde07.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
同时，`isinstance`还接受一个包含类型的无组，你可以检查对象的类型是否在无组中的类型中：
![](https://upload-images.jianshu.io/upload_images/15777037-e8394634ae862e44.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

### Python中的对象和方法
对象和方法的调用和Java一致，通过`obj.attribute_name`的语法进行调用，如下：
![](https://upload-images.jianshu.io/upload_images/15777037-4d073d0040b9134c.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
同时，属性和方法也可以通过`getattr`函数获得：
![](https://upload-images.jianshu.io/upload_images/15777037-9f0c286c795f564d.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
### 导入
在Python中，模块就是以`.py`为后缀并包含Python代码的文件，假设我们有以下模块：
![](https://upload-images.jianshu.io/upload_images/15777037-83244e6e55d148dd.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
假如我们想从另一个`相同路径下`的文件连接到这个模块中定义的变量和函数，可以如下：
![](https://upload-images.jianshu.io/upload_images/15777037-354bf1acd856072d.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

### 二元运算符和比较运算
所有可用的二元运算符如下：
![](https://upload-images.jianshu.io/upload_images/15777037-50fdeab68cf7508d.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
其中，`is`关键字，用于检查两个引用是否指向同一个对像，`is not`则检查指向不同对象,比较`is`和`==`的不同，如：
![](https://upload-images.jianshu.io/upload_images/15777037-ff5cbfe41f17629d.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
![](https://upload-images.jianshu.io/upload_images/15777037-faf1f3d888339916.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
+ **数值类型**：包括`int`和`float`，`int`可以存储任意大小数字，`float`表示浮点数，每一个浮点数是双精度64位数值，也可以使用科学计数法表示，如下：
![](https://upload-images.jianshu.io/upload_images/15777037-2ef6eff0e7e735db.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
`//`表示整除操作
+ **字符串**：可以用单引号或双引号写一个字符串，对于含有换行的多行字符串，可以使用三个单引号或三个双引号,我们可以通过统计`\n`的个数，来看一下c里面包括了几行字符串，如下
![](https://upload-images.jianshu.io/upload_images/15777037-85f598c2031f9f1f.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
可以通过`str`函数转成字符串。
由于字符串是Unicode字符的序列，因此可以看作一个序列，如下：
![](https://upload-images.jianshu.io/upload_images/15777037-4749d50a0f412157.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
...
![](https://upload-images.jianshu.io/upload_images/15777037-bf83ec8f15660a6e.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
在文件的上下文中，最常遇到的就是bytes对象，在字符串前加前缀`b`来定义字符文本，可以让该文本为`utf-8`编码：
![](https://upload-images.jianshu.io/upload_images/15777037-d43e4fcd0168ec54.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
+ **布尔值**：Python中的布尔值写作True和False（首字母需要大写）
+ **类型转换**：str、bool、int和float就是数据类型，同时也可以将其他数据转换为这些类型的函数：
![](https://upload-images.jianshu.io/upload_images/15777037-76d934638035a5ee.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
+ **None**是Python的null值类型，如果一个函数没有显式地返回值，则它会隐式地返回None，从技术角度来说，None不仅是一个关键字，它还是NoneType类型的唯一实例：
![](https://upload-images.jianshu.io/upload_images/15777037-c32b4d0c0a7fd132.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
+ **时间和日期**：Python中内建的datetime模块，提供了datetime、date和time类型，用法如下：
![](https://upload-images.jianshu.io/upload_images/15777037-fc972c1ce1cea8fc.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
`strftime`方法将datetime转换为字符串，`strptime`将字符串转化为datetime对象：
![](https://upload-images.jianshu.io/upload_images/15777037-e728541d267aab9f.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
完整的格式化说明：
![](https://upload-images.jianshu.io/upload_images/15777037-28b03b25ab68e180.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
使用`replace`方法，替换datetime时间序列中的一些值，比如将分钟、秒替换为0：
![](https://upload-images.jianshu.io/upload_images/15777037-f4174e9eb1ea18fa.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
由于datetime.datetime是不可变类型，以上方法都是产生新的对象，两个不同的datetime对象会产生一个`datetime.timedelta`类型的对象：
![](https://upload-images.jianshu.io/upload_images/15777037-e6d3f0f32e170193.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
输出的timedelta()表示时间间隔为17天7179秒，将timedelta加到一个datetime上将产生一个新的对象
![](https://upload-images.jianshu.io/upload_images/15777037-fd2a11f06b4fe63e.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
